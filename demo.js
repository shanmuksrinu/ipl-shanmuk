let data = {
  ...
  //question 1 is payment methods in array.
  //2 delivery area inbetween a-n
  //menu item of Desserts names in array.
 ...
 {
     if(prop[i]=="restaurant-info")
     {
         var check=data[prop[i]];
         var result=Object.keys(check);
         for(var j=0;j<result.length;j++)
         {
            if(result[j]=="deliverable-area")
            {
               data[prop[i]][result[j]].map((x)=>{ 
                var reg=/^[a-n]/i
                if(reg.test(x.name))
                {
                    ques_2.push(x.name)
            ...
            if(result[j]=="payment-methods")
            {
                data[prop[i]][result[j]].map((x)=>{ques_1.push(x.type)});
            }
     ...
     if(prop[i]=="categorys")
     {
        var final=Object.keys(data[prop[i]]);
        for(var z=0;z<final.length;z++)
        {
            if(data[prop[i]][final[z]].name=="Dessert")
            {
                var final_check=data[prop[i]][final[z]]['menu-items'];
                final_check.map((x)=>{
                    x["sub-items"].map((y)=>ques_3.push(y.name));
